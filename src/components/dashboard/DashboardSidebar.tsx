...
export function DashboardSidebar({
  ...
  return (
    <aside
      className={`${className} theme-panel shrink-0 flex-col border-r border-border/80 backdrop-blur-xl transition-all duration-300 ${
        collapsed ? "w-[72px]" : "w-64"
      }`}
    >
      <div className={`flex items-center border-b border-border/60 ${collapsed ? "justify-center p-4" : "justify-between p-5"}`}>
        {!collapsed && (
          <div className="min-w-0">
            <p className="text-[10px] uppercase tracking-[0.35em] text-accent">Member Portal</p>
            <p className="font-display truncate text-sm text-foreground">{user.displayName}</p>
            <p className="truncate text-xs text-muted">{user.membershipLabel}</p>
          </div>
        )}
        <button
          onClick={onToggle}
      ...
      {user.role === "admin" && (
        <div className="border-t border-border/60 p-3">
          <Link
            ...
            className={`flex items-center gap-3 rounded-[12px] border border-accent/25 bg-accent/10 px-3 py-3 text-sm tracking-wide text-accent transition-all duration-300 hover:bg-accent/20 ${
              collapsed ? "justify-center" : ""
            }`}
          >
      ...
      <div className="border-t border-border/60 p-3">
        <button
          ...
          className={`flex w-full items-center gap-3 rounded-[12px] px-3 py-3 text-sm tracking-wide text-muted transition-all duration-300 hover:bg-red-500/10 hover:text-red-400 ${
            collapsed ? "justify-center" : ""
          }`}
        >
          <svg className="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
